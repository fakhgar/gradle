...
class NodeState implements DependencyGraphNode {
    ...
    private PotentialEdge potentialEdgeTo(ModuleComponentIdentifier toComponent, ModuleComponentSelector toSelector) {
        DependencyState dependencyState = new DependencyState(new VirtualPlatformDependencyMetadata(toSelector, toComponent), resolveState.getComponentSelectorConverter());
        EdgeState edge = new VirtualEdge(dependencyState);
    ...
    private void addPlatformEdges(Collection<EdgeState> discoveredEdges, ModuleComponentIdentifier platformComponentIdentifier, ModuleComponentSelector platformSelector) {
        ...
        if (metadata == null || metadata instanceof VirtualPlatformMetadata) {
        ...
        if (metadata == null) {
            ...
            metadata = new VirtualPlatformMetadata(platformComponentIdentifier, potentialEdge.toModuleVersionId, virtualPlatformState.getParticipatingComponents());
            potentialEdge.component.setMetadata(metadata);
        }
        // note: we don't need to add the edge to "outgoingEdges" because we don't need to see them
        // in the resolution result as "real edges"
        discoveredEdges.add(potentialEdge.edge);
        potentialEdge.edge.getSelector().use();
    ...
    private class VirtualPlatformDependencyMetadata implements ModuleDependencyMetadata {
        private final ModuleComponentSelector cs;
        private final ModuleComponentIdentifier platformId;

        VirtualPlatformDependencyMetadata(ModuleComponentSelector cs, ModuleComponentIdentifier platformId) {
            this.cs = cs;
        ...
        public List<ConfigurationMetadata> selectConfigurations(ImmutableAttributes consumerAttributes, ComponentResolveMetadata targetComponent, AttributesSchemaInternal consumerSchema) {
            if (targetComponent instanceof VirtualPlatformMetadata) {
                VirtualPlatformMetadata platformMetadata = (VirtualPlatformMetadata) targetComponent;
                return Collections.<ConfigurationMetadata>singletonList(new VirtualConfigurationMetadata(platformMetadata.moduleVersionIdentifier.getVersion(), platformMetadata.getParticipatingComponents()));
            }
        ...
        private class VirtualConfigurationMetadata extends DefaultConfigurationMetadata {

            private final String targetVersion;
            private final Set<ComponentState> participatingComponents;

            VirtualConfigurationMetadata(String targetVersion, Set<ComponentState> participatingComponents) {
                ...
                for (ComponentState componentState : participatingComponents) {
                    if (componentState.isSelected() && !componentState.getId().getVersion().equals(targetVersion)) {
                        ...
                        if (potentialEdge.metadata != null) {
                            ...
                            result.add(new VirtualPlatformDependencyMetadata(
                                leafSelector,
                                leafId
                            ));
                        }
    ...
    private class VirtualPlatformMetadata implements ModuleComponentResolveMetadata {

        private final ModuleComponentIdentifier moduleComponentIdentifier;
        private final ModuleVersionIdentifier moduleVersionIdentifier;
        private final Set<ComponentState> participatingComponents;

        private VirtualPlatformMetadata(ModuleComponentIdentifier moduleComponentIdentifier, ModuleVersionIdentifier moduleVersionIdentifier, Set<ComponentState> participatingComponents) {
        ...
        public Set<ComponentState> getParticipatingComponents() {
            return participatingComponents;
        }
    }

    private class VirtualEdge extends EdgeState {
        public VirtualEdge(DependencyState dependencyState) {
